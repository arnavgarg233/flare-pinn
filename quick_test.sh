#!/bin/bash
# Quick CPU Smoke Test - Run both baseline and physics-informed
# Takes ~30-45 minutes total on CPU
...
# Test 1: Data-only baseline (no physics)
echo "Step 1/2: Training BASELINE (data-only, no physics)..."
python src/train_pinn_improved.py \
  --config configs/train_baseline_data_only.yaml \
  --log logs/smoke_test_baseline.log
...
# Test 2: Physics-informed
python src/train_pinn_improved.py \
  --config configs/train_pinn_cpu_smoke_test.yaml \
  --log logs/smoke_test_physics.log
